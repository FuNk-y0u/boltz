...
from src.constants import DOWNLOAD_LIST
...
def find_and_download_songs(token,kwargs):
    """
    ...
    with open(reference_file, "r", encoding="utf-8") as file:
        ...
        for line in file:
            ...
            totalprog = round((count/total)*100)
            update_state(token, totalprog, name, False)
            ...
            count += 1
    update_state(token, totalprog, name, True)
    logOk(f"Sucessfully Downloaded")     
...
def download_songs(token,**kwargs):
    """
    ...
    for url in kwargs["songs"]["urls"]:
        logHeader("saving to " +  str(url["save_path"]))
    ...
    find_and_download_songs(token,kwargs)
